...
int len_expand(char *prompt, t_tks *tks, char **env)
{
	...
	{
		if (prompt[id->i] == '$' && tks->dol[id->j++] && no_expand_hd(prompt, id->i, tks))
		{
			id->start = ++id->i;
			while (prompt[id->i] && prompt[id->i] != ' ' && !is_token(prompt[id->i]))
				id->i++;
...
int no_expand_hd(char *prompt, int dol, t_tks *tks)
{
	...
	{
		if (prompt[dol] == '<' && prompt[dol - 1] && prompt[dol - 1] == '<' && (!in_quotes(tks, prompt, dol)))
			return (0);
